...
query ─┬─ BM25: in-index tokenizer, no model ───────────────┐
       ...
       └─ dense: WASM candle (bert) | transformers.js (WebGPU) ┘
```
...
- RRF, `k = 60`, per-arm weights (defaults since 0.4.1, from graded sweeps on two labelled sites: dense 1.2, sparse 0.8, bm25 0.6; bm25 1.0 when there is no sparse arm). `fetch_k = max(3 × top_k, 30)` per arm, fused at chunk level.
