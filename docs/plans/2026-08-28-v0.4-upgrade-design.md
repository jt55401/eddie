...
- RRF, `k = 60`, per-arm weights (defaults: dense 1.0, sparse 1.0, bm25 0.8; bm25 1.0 when there is no sparse arm). `fetch_k = max(3 × top_k, 30)` per arm, fused at chunk level.
