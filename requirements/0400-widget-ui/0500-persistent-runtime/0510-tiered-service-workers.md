...
- Accepting a `wasm-candle` or `webgpu-onnx` search lane registers or upgrades only the service worker tier that lane needs, and fetches nothing belonging to another tier: accepting the WebGPU search lane fetches `eddie-sw-gpu.js` and `eddie-transformers-sw.js` and no WebLLM.
- Accepting the agent registers `eddie-sw-agent.js` in the `sw/agent/` scope, independently of whether any search lane was accepted, and leaves the search tier where it was.
...
- `docs/plans/2026-09-01-efficient-defaults-pass2.md` — measured per-tier fetch sets
