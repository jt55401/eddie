...
// eddie patch: simd128 has no CurrentCpuF16/CurrentCpuBF16, so the f16/bf16
// vector adds must use the scalar fallback on wasm32 (upstream candle 0.11.0
// fails to compile with `-C target-feature=+simd128` otherwise).
...
// eddie patch: see vec_add_f16 above.
#[cfg(any(target_feature = "neon", target_feature = "avx2"))]
#[inline(always)]
